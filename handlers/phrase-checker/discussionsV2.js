...
//########################################################################################################################################//
///// THIS FUNCTION IS DESIGNED TO FIND ALL THE DISCUSSION PAGES IN A COURSE AND ADD THEIR URL TO THE 'responseArray' TO LATER LOOP OVER///////
//########################################################################################################################################//
...
var apiGet = function(domain, checkObject, string, userCourseNumber, replaceString, update, replaceCaseSensitive, searchCaseSensitive) {
    ...
    this.apiCall = function() {
        ...
        var getList = {
            url: "https://" + domain + ".instructure.com/api/v1/courses/" + userCourseNumber + "/discussion_topics?per_page=100&page=" + paginatedPage,
        ...
        request(getList, function(error, response, body) {
            ...
            if (ApiResponse.length > 0) {
                ...
                console.log("\n \n");
                console.log("Done finding all of the", style.color.ansi16m.hex("#ff0000 ") + "Discussion Topics" + style.color.close + " in this course.");
                console.log("I found a total of", responseArray.length, "Discussions.");
                console.log("\n \n");
...
//########################################################################################################################################//
/// THIS FUNCTION GOES THROUGH THE DISCUSSION TOPICS AND LOOKS FOR A MATCH BETWEEN THE SEARCH STRING AND WHAT IS ON THE DISCUSSIONS BODY ///
//########################################################################################################################################//
...
async function dataFinder() {
    ...
    let array = responseArray.map(id => {
        ...
        var options = {
            method: "get",
            url: "https://" + Domain + ".instructure.com/api/v1/courses/" + courseNumber + "/discussion_topics/" + id,
        ...
        async function getData() {
            ...
            let data = await axios(options).then(function(response) {
                 ...
                 let titleURL = response.data.id;
                 let body = response.data.message;
                 let title = response.data.title;
                 let normalPageUrl = "https://" + Domain + ".instructure.com/courses/" + courseNumber + "/discussion_topics/" + titleURL;
                 let bodyStringify;
                 let userStringify
                            ...
                            if(bodyCheck.indexOf(stringCheck) !== -1){
                                var newBody = body.replace(searchString, ReplaceString)
                                    if(updateMe === true){
                                        var updateOptions = {
                                            method: "put",
                                            url: "https://" + Domain + ".instructure.com/api/v1/courses/" + courseNumber + "/discussion_topics/" + titleURL,
