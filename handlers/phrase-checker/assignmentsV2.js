...
//########################################################################################################################################//
///// THIS FUNCTION IS DESIGNED TO FIND ALL THE ASSIGNMENTS IN A COURSE AND ADD THEIR URL TO THE 'responseArray' TO LATER LOOP OVER///////
//########################################################################################################################################//
    ...
    this.apiCall = function() {
        ...
        request(getList, function(error, response, body) {
            ...
            for (i = 0; i < ApiResponse.length; i++) {
                    if(!ApiResponse[i].is_quiz_assignment){
            ...
            if (ApiResponse.length > 0) {
                ...
                console.log("\n \n");
                console.log("Done finding all of the", style.color.ansi16m.hex("#ff0000 ") + "Assignments" + style.color.close + " in this course.");
                console.log("I found a total of", responseArray.length, "Assignments.");
                console.log("\n \n");
...
//########################################################################################################################################//
/// THIS FUNCTION GOES THROUGH THE ASSIGNMENTS AND LOOKS FOR A MATCH BETWEEN THE SEARCH STRING AND WHAT IS ON THE ASSIGNMENTS BODY ///
//########################################################################################################################################//
...
async function dataFinder() {
    ...
    let array = responseArray.map(id => {
        ...
        var options = {
            method: "get",
            url: "https://" + Domain + ".instructure.com/api/v1/courses/" + courseNumber + "/assignments/" + id,
        ...
        async function getData() {
            ...
            let data = await axios(options).then(function(response) {
                 ...
                 let normalPageUrl = "https://" + Domain + ".instructure.com/courses/" + courseNumber + "/assignments/" + titleURL;
                 let bodyStringify;
                 let userStringify
                                ...
                                var newBody = body.replace(searchString, ReplaceString)
                                    if(updateMe === true){
                                        var updateOptions = {
                                            ...
                                            data:{"assignment": {"description": newBody}}
                                        };
